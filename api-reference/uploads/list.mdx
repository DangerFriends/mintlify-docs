...
```bash
curl "https://api.fon.app/api/uploads?page=1&limit=12"
```
...
```bash
curl "https://api.fon.app/api/uploads/filtered?tags=gadget,tech&page=1"
```
...
```bash
curl "https://api.fon.app/api/uploads/filtered?tags=gadget&excludeUploadId=current-upload-uuid"
```
...
```bash
curl https://api.fon.app/api/items-recent
```
...
```bash
curl "https://api.fon.app/api/items-top-voted?limit=10&timeRange=week"
```
