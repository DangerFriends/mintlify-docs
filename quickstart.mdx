...
Before using the API, you need a FoN account. Register at [fon.app](https://fon.app) or use the API:
...
```bash
curl https://api.fon.app/api/uploads?page=1&limit=10
```
...
async function uploadImage(filePath, description) {
  ...
  const response = await fetch('https://api.fon.app/api/v1/upload', {
    method: 'POST',
    headers: {
...
def upload_image(file_path, description):
    with open(file_path, 'rb') as f:
        response = requests.post(
            'https://api.fon.app/api/v1/upload',
