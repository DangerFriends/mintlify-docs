...
```bash
curl -X POST https://api.fon.app/api/auth/tokens/generate \
...
```bash
curl -X DELETE https://api.fon.app/api/auth/tokens/TOKEN_ID \
  -H "Authorization: Bearer YOUR_JWT_TOKEN"
```
...
```bash
curl -X POST https://api.fon.app/api/auth/mfa/setup \
  -H "Authorization: Bearer YOUR_JWT_TOKEN"
```
...
<Steps>
  ...
  <Step title="Verify Setup">
    ...
    ```bash
    curl -X POST https://api.fon.app/api/auth/mfa/verify \
      -H "Authorization: Bearer YOUR_JWT_TOKEN" \
      -H "Content-Type: application/json" \
      -d '{"mfaCode": "123456"}'
    ```
